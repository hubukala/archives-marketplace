...
const ConfirmPurchaseForm = ({ productId }) => {
    ...
    return(
        <FormContainer>
            <FormElement action="" className="form" onSubmit={formik.handleSubmit}>
                <h3>Shipping adress</h3>
                <InputMain type="text" name="city" placeholder="City" onChange={formik.handleChange} value={formik.values.city}/>
                {formik.touched.city && formik.errors.city ? (
                    <InputError>{formik.errors.city}</InputError>
                ) : null}
                <InputMain type="text" name="zipcode" placeholder="Zip code" onChange={formik.handleChange} value={formik.values.zipcode}/>
                {formik.touched.zipcode && formik.errors.zipcode ? (
                    <InputError>{formik.errors.zipcode}</InputError>
                ) : null}
                <InputMain type="text" name="adress" placeholder="Street adress" onChange={formik.handleChange} value={formik.values.adress}/>
                {formik.touched.adress && formik.errors.adress ? (
                    <InputError>{formik.errors.adress}</InputError>
                ) : null}
                <h3>Payment Details</h3>
                <InputMain type="text" name="cardnumber" placeholder="Card number" onChange={formik.handleChange} value={formik.values.cardnumber}/>
                {formik.touched.cardnumber && formik.errors.cardnumber ? (
                    <InputError>{formik.errors.cardnumber}</InputError>
                ) : null}
                <InputMain type="text" name="expirationdate" placeholder="Expiration date (example: MM/YY)" onChange={formik.handleChange} value={formik.values.expirationdate}/>
                {formik.touched.expirationdate && formik.errors.expirationdate ? (
                    <InputError>{formik.errors.expirationdate}</InputError>
                ) : null}
                <InputMain type="text" name="cvv" placeholder="CVV" onChange={formik.handleChange} value={formik.values.cvv}/>
