...
const ProfilePage = () => {
    ...
    const handleAvatarUpload = (event) => {
        ...
        const imageRef = ref(
            storage,
            `avatars/${event.target.files[0].name + uniqueId}`,
        );
            ...
            {!signedInUser ? (
                ...
                <ProfileContainer>
                    <ProfileHead>
                        ...
                        Joined in:{' '}
                        {signedInUser.metadata.creationTime.slice(12, 17)}
                    </ProfileHead>
                    <MainContainer>
                        <ProfileSideBar>
                            <SideBarButton
                                onClick={() => changeMain('account')}
                            >
                                ACCOUNT
                            </SideBarButton>
                            <SideBarButton onClick={() => changeMain('orders')}>
                                ORDERS
                            </SideBarButton>
                            <SideBarButton
                                onClick={() => changeMain('my-items')}
                            >
                                MY ITEMS
                            </SideBarButton>
                        </ProfileSideBar>
                        <ProfileMain>{renderMain()}</ProfileMain>
                    </MainContainer>
                </ProfileContainer>
            )}
        </>
